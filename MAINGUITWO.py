...
acc_pass=tk.StringVar()
load_amt=tk.StringVar()
pass_w=tk.StringVar()
phno = tk.StringVar()
trans_amt=tk.StringVar()
rece_accnum=tk.StringVar()
sndr_accpass=tk.StringVar()
fname = tk.StringVar()
lname = tk.StringVar()
phno = tk.StringVar()
email = tk.StringVar()
...
def frame_text(t):
    frame_label= customtkinter.CTkLabel(master=app, text=t).grid(row=4,column=2)
...
def retrieve_data():
    ...
    def cal_mny(arg):
        ...
        if(arg=="+"):
            fun(int(sender_total_amount)+int(transfering_amount))
        elif(arg=="-"):
            fun(int(sender_total_amount)-int(transfering_amount))
    ...
    def load_money():
        l_m(customtkinter.CTkButton(master=app, text="Confirm",command=lambda:[frame_text("Transaction Done Sucessfully"),cal_mny("+")],width=170,height=30).grid(row=3, column=1, padx=15, pady=0))
